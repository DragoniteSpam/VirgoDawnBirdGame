...
while (!ds_queue_empty(q)){
    ...
    if (validate_boolean(token)&&token!="+"&&token!="-"){
        ds_queue_enqueue(output_q, token);
    } else if (validate_function(token)){
        ds_stack_push(operator_stack, token);
    } else if (validate_boolean_operator(token)){
        while (ds_stack_size(operator_stack)>0&&((validate_function(ds_stack_top(operator_stack))||
                eval_boolean_operator_precedence(ds_stack_top(operator_stack))>eval_boolean_operator_precedence(token))||
                eval_boolean_operator_precedence(ds_stack_top(operator_stack))==eval_boolean_operator_precedence(token)&&eval_boolen_left_associative(token)&&
...
while (!ds_queue_empty(output_q)){
    ...
    if (string_length(string_trim(token))>0){
        if (validate_boolean(token)){
            ds_stack_push(onemorestack, to_boolean(token));
        } else if (ds_map_exists(World.variables, token)){
            if (validate_boolean(World.variables[? token])){
                ds_stack_push(onemorestack, real_boolean(World.variables[? token]));
            } else {
                eval_boolean_error("Wrong type of variable :#"+token+"#in#"+argument0);
            }
//        } else if (ds_map_exists(World.cutscene_eval_boolean_operators, token)){
...
#define eval_boolean_operator_precedence
/// double eval_boolean_operator_precedence(operator);
...
#define eval_boolean_error
// Theoretically you could show some more information here.

show_message(argument0);

#define eval_boolen_left_associative
// I don't know if this can return false but here's a script for it anyway.
