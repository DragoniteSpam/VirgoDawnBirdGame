...
d3d_model_draw_selective(ds_map_find_value(all_mod_packages, BASE_ASSET).my_models[World.mdl_clock_gear_small].model_array[0], 0, 0, 0, -1, BASE_ASSET, World.mdl_clock_gear_small);
    ...
    if (obj!=caster){
        with (obj){
            if (hit_flag_accept(c_hit_x(), c_hit_y())){
                if (script_execute(all_hit_scripts[on_hit], CombatTypes.smash, other.caster, other.strength)){
                } else {
